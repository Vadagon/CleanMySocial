...
async function checkProductPages() {
  ...
  await inBatches(routes, async (route) => {
    ...
    try {
      ...
      // The public 3-day pass, Monthly, and Lifetime must all be present in the
      // initial HTML so buyers see every option every day.
      const radioCount = (html.match(/role="radio"/g) || []).length;
      if (radioCount !== 3) fail(scope, `expected 3 public plans, found ${radioCount}`);
